...
pygame.init()
pygame.mixer.init()
pygame.mixer.music.load("TSIS4/assets/sounds/track1.mp3")
...
#Setup database and handle any exceptions, fallback to JSON if DB setup failsa
try:
    db.setup_database()
    print("Database ready")
except Exception as e:
    print("DB setup failed, continuing with JSON fallback. Error:", e)


#Function to draw a button and return whether it's being hovered
...
def main():
    ...
    while running:
        ...
        #Draw background and UI based on current state
        ...
        try:
            ...
            player_id = None
            print("DB get_or_create_player failed, will use JSON fallback:", e)

        #Game over screen showing final score and options to retry or go back to menu
        if state == "GAME_OVER":
            ...
            #Button logic for retrying the game or going back to the menu
        ...
        #Leaderboard screen showing top 10 scores from the database
        elif state == "LEADERBOARD":
            title = font.render("TOP 10 SCORES", True, (255, 215, 0))
            screen.blit(title, (WIDTH//2 - title.get_width()//2, 30))

            #Fetch top 10 scores from the database and display them
            y_offset = 100
        ...
        #Settings screen allowing users to toggle grid, sound, and cycle snake colors
        elif state == "SETTINGS":
